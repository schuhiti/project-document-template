...
3. 生成された中身を新しいプロジェクトにコピーする。コピー先に同名ファイルが既にあれば上書きしない。設定・宣言ファイルは既存内容を確認した上で可能なら統合する（`.gitattributes`は`*.sh`の改行コードをLFに固定するだけで通常は問題にならないため、`*.sh`関連の記述をgrepで確認し無ければ追記する。`.claude/settings.json`は`hooks`オブジェクトを統合する）。`CLAUDE.md`・`AGENTS.md`のように自然文で書かれ機械的に統合できないファイルや、統合方法が自明でない場合はコピーを中止し人間に確認する
4. `SETUP.md` の手順に従う
