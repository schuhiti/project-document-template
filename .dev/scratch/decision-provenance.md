...
- **hookがBash経由の編集を捉えない**。matcherが`Write|Edit`なので、`sed`・`python`等をBashツールで走らせて書き換えた場合はどのhookも発火しない。このセッションの編集はすべてBash経由で、frontmatter・index整合・孤立検査のいずれも一度も動いていない。matcherに`Bash`を足しても、コマンド文字列から書き換え先のパスを機械的に取り出せないため同じ形では作れない。トリガー: Bash経由の編集で規約違反を実際に取りこぼした時
