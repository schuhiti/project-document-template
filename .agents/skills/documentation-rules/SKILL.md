...
- 経緯・履歴は基本的に書かない。理由を書く価値があるのは、標準・既定の振る舞いから外れる選択をした場合のみ（標準・既定に従っただけの選択には理由を書かない）。書く場合の扱いは「決定・経緯を書く際の分量基準」を参照
...
- ある条件を示せば否定・補集合が自動的に導ける場合、補集合を重ねて書かない（例:「Xなら進めてよい」と書けば「Xでなければ保留する」は自明であり、重ねて書く必要はない）
- 同じ主題語が短い範囲で繰り返し登場する場合、主題を一度立てて以降は省略する（日本語の主題省略を活用し、毎回名詞を書き直さない）
