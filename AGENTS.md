...
4. `.claude/skills` が無ければ作る: `mkdir -p .claude && ln -s ../.agents/skills .claude/skills`。作成したら、以後の環境でも再現するようgitへのコミットを推奨する（Linux/macOSはリンクがそのままコミットできる）。作成できない場合（Windowsで管理者権限が無い等）は、その旨を記録し、作成できる環境で再確認する。Claude CodeがAGENTS.mdへ直接対応する等、ツールの対応状況が変わった場合はこの項目全体を見直す
